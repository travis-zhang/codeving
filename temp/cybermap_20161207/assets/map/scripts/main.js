...
function System(id, name, description, color, sides) {
    // ID
    this.id = id;
    this.name = name;
    ...
    // counter elements
    this.el_count_selector = '.type-icons .symbol.' + name.toLowerCase() + ' .count';
    this.el_popcount_selector = '#' + name.toLowerCase() + '-popcount';
    this.el_popcount = $('#' + name.toLowerCase() + '-popcount');
};
...
MAP.init = function (opts) {
    ...
    _.forEach(opts.systems, function (s) {
            GTW.systems[s[0]] = new System(s[0], s[1], s[2], s[3], s[4]);
        }
    );
    ...
    function set_popup_country(country) {
        ...
        if (country) {
            UI.show_country_popup(GTW.get_country_name(country));
            localStorage.setItem("selectedCountry", country.iso2);
    ...
    var missiles = new GTW.MissileSystem(env, systems);
    ...
    var el_popup_country_rank = $('#countrypop_ranking');
    ...
    function update_missiles(events) {
        ...
        _.each(events, function (ke) {
            ...
            launch_missile(system_id, target_key, source_key, ke.coords, ke.angle);
        });
        ...
        if (popup_country && (next_rank_update_time < env.time)) {
            ...
            el_popup_country_rank.text(GTW.total_target_rank[key]);
            next_rank_update_time = env.time + 2.0;
        }
        ...
        if (next_counter_update_time < env.time) {
            GTW.systems_foreach(function (system) {
                $(system.el_count_selector).text(system.count);
                if (popup_country) {
                    var key = popup_country.key;
                    var count = system.target_count[key];
                    $(system.el_popcount_selector + ' h4').text(count);
                }
            });
    ...
    function change_animation_state(new_state, force) {
        ...
        switch (new_state) {
            ...
            case 'solo':
                hedgehog.hide();
                var sysIdArray = [];
                GTW.systems_foreach(function (sys, id) {
                    if (sys.enabled) {
                        sysIdArray.push(id);
                    }
                });
                if (sysIdArray.length > 0) {
                    ...
                    if (showSubsystemPopup) {
                        solo_system(env.solo_system_id);
                    }
                    else {
                        solo_system(null);
                    }
    ...
    function resize() {
        ...
        if (is_widget || is_screensaver) {
            ...
            if (platformDetection.isMobile) {
                canvas.height = parseInt($(window).innerHeight()) - (parseInt($('.header').height()) + 45);
            } else {
                canvas.height = parseInt($(window).innerHeight()) - (parseInt($('.header').height()) + parseInt($('#footer').height()));
            }
        }
    ...
    // UI interface
    function toggle_system_prop(system_name, prop_name, value) {
        system_name = system_name.toUpperCase();
        var system = _.where(GTW.systems, {name: system_name})[0];
        //console.assert(system);
        if (typeof value == 'undefined')
            return system[prop_name] = !system[prop_name];
        else
            return system[prop_name] = value;
    }
...
//            console.log('MAP.pause');
        ...
        attach_graph_canvas: function (el) {
            //  console.log('MAP.attach_graph_canvas', el);
    ...
    function solo_system_line(sysid) {
        if (MAP.is_bad_mode) {
            return;
            // no system lines
        }
        ...
        // call function again to handle resize
        if (sysid) {
            // define some usefull vars
            var sys = GTW.systems[sysid];
            var type = sys.name.toLowerCase();
            var startpoint = {
                top: Math.round($(window).height() * .7),
                left: Math.round($(window).width() * .2)
            };
            var endpoint = $('.type-icons .symbol.' + type).offset();
            endpoint.top = endpoint.top - 35;
            var canvasWidth = 0;
            var canvasHeight = Math.round(endpoint.top - startpoint.top);

            var canvas = $('#systempop_line')[0];
            if (!canvas) {
                console.log('solo_system_line: no canvas');
                // mobile doesn't seem to have it?
                return;
            }

            var canvasContext = canvas.getContext('2d');

            endpoint.left = endpoint.left + (parseInt($('.type-icons .symbol.' + type).outerWidth()) / 2); // to account for the size of the icon blocks
            if (endpoint.left > startpoint.left) {
                canvasWidth = Math.round(endpoint.left - startpoint.left);
            } else if (endpoint.left < startpoint.left) {
                canvasWidth = Math.round(startpoint.left - endpoint.left);
            } else {
                canvasWidth = 1;
            }

            // size and position the canvas
            $('#systempop_line').attr('width', canvasWidth).attr('height', canvasHeight).css({
                width: canvasWidth,
                height: canvasHeight,
                top: startpoint.top
            });
            if (endpoint.left > startpoint.left) {
                $('#systempop_line').css({left: startpoint.left});
            } else if (endpoint.left < startpoint.left) {
                $('#systempop_line').css({left: endpoint.left});
            } else {
                $('#systempop_line').css({left: startpoint.left});
            }
            ...
            if (endpoint.left > startpoint.left) {
                canvasContext.moveTo(0.5, 0);
                canvasContext.lineTo(0.5, Math.floor(canvasHeight / 2) + 0.5);
                canvasContext.lineTo(canvasWidth - 0.5, Math.floor(canvasHeight / 2) + 0.5);
                canvasContext.lineTo(canvasWidth - 0.5, canvasHeight);
            } else if (endpoint.left < startpoint.left) {
                canvasContext.moveTo(canvasWidth - 0.5, 0);
                canvasContext.lineTo(canvasWidth - 0.5, Math.floor(canvasHeight / 2) + 0.5);
                canvasContext.lineTo(0.5, Math.floor(canvasHeight / 2) + 0.5);
                canvasContext.lineTo(0.5, canvasHeight);
            } else {
                canvasContext.moveTo(0.5, 0);
                canvasContext.lineTo(0.5, canvasHeight);
            }
    ...
    function solo_system_line_handle_resize() {
        var foundLine = false;
        GTW.systems_foreach(function (sys, id) {
            if (sys.enabled && $('#systempop').is(":visible")) {
                solo_system_line(+id);
                foundLine = true;
            }
        });
        if (!foundLine) {
            solo_system_line(null);
        }
    }

    function solo_system(sysid) {
        // list of all system types (oas ods ...)
        var system_classname_list = (function () {
            var names = [];
            GTW.systems_foreach(function (sys) {
                names.push(sys.name.toLowerCase());
            });
            return names.join(' ');
        }());
        ...
        if (sysid) {
            ...
            $('#systempop').removeClass(system_classname_list).empty().html('');
            $('#systempop').addClass(type).append(content).fadeIn();
        } else {
            $('#systempop').fadeOut(400, function () {
                $('#systempop').removeClass(system_classname_list).empty().html('');
            });
        }
        solo_system_line(sysid);
    }
    ...
    world.on('loaded', function () {
        ...
        if (startFromSelectedCountry && localStorage.getItem("selectedCountry") !== null)
        {
            iso2 = localStorage.getItem("selectedCountry");
    ...
    $(window).resize(function () {
        solo_system_line_handle_resize();
    });

    // BAD mode
    if (MAP.is_bad_mode) {
        GTW.systems_foreach(function (sys) {
            sys.enabled = (sys.name == 'BAD');
            change_animation_state('spin_1');
        });
    }
